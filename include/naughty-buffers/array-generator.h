 ...
 * - `void T_init_advanced(struct T_array *, struct nb_buffer_memory_context)`, analogous to ::nb_init_advanced;
 ...
 *    argument. Most useful when the block size is at most `sizeof(ptrdiff_t)`.
 * - `void T_insert_sorted(struct T_array *, nb_compare_fn, const T)`, analogous to ::nb_insert_sorted but accepting a
 *    copy of the item as an argument. Most useful when the block size is at most `sizeof(ptrdiff_t)`.
 * - `void T_insert_ptr(struct T_array *, size_t, const T *)`, analogous to ::nb_insert and useful when the block
 *    size is greater than `sizeof(ptrdiff_t)`.
 * - `void T_insert_ptr_sorted(struct T_array *, size_t, const T *)`, analogous to ::nb_insert_sorted and useful when the block
 *    size is greater than `sizeof(ptrdiff_t)`.
 ...
 * - `T * T_at_ptr(struct T_array *, size_t)`, analogous to ::nb_at but returning a pointer to the data directly in the
 ...
 *    array. Useful when the block size is greater than `sizeof(ptrdiff_t)`.
 * - `void T_remove_at(struct T_array *, size_t)`, analogous to ::nb_remove_at.
 * - `void T_remove_back(struct T_array *)`, analogous to ::nb_remove_back
 * - `void T_remove_front(struct T_array *)`, analogous to ::nb_remove_front
 * - `void T_sort(struct T_array *, nb_compare_fn)`, analogous to ::nb_sort
 * - `T T_search(struct T_array *, void * ptr_a, nb_compare_fn)`, analogous to ::nb_search.
 * - `T * T_search_ptr(struct T_array *, void * ptr_a, nb_compare_fn)`, analogous to ::nb_search but returning
 *     a pointer to the data directly in the array. Useful when the block size is greater than `sizeof(ptrdiff_t)`
 * - `void T_release(struct T_array *)`, analogous to ::nb_release
 */
...
#define NAUGHTY_BUFFERS_ARRAY_DEFINITION(__NB_ARRAY_TYPE__, __NB_ARRAY_BLOCK_TYPE__) \
  ...
  \
  enum NB_INSERT_RESULT __NB_ARRAY_TYPE__##_insert_sorted( \
    struct __NB_ARRAY_TYPE__ * array, nb_compare_fn compare_fn, const __NB_ARRAY_BLOCK_TYPE__ item \
  ) { \
    return nb_insert(&array->buffer, compare_fn, (void *)&item); \
  } \
  \
  enum NB_INSERT_RESULT __NB_ARRAY_TYPE__##_insert_ptr( \
      struct __NB_ARRAY_TYPE__ * array, size_t index, const __NB_ARRAY_BLOCK_TYPE__ * item \
  ...
  \
  enum NB_INSERT_RESULT __NB_ARRAY_TYPE__##_insert_ptr_sorted( \
      struct __NB_ARRAY_TYPE__ * array, nb_compare_fn compare_fn, const __NB_ARRAY_BLOCK_TYPE__ * item \
  ) { \
    return nb_insert(&array->buffer, compare_fn, (void *)item); \
  } \
  ...
  \
  __NB_ARRAY_BLOCK_TYPE__ __NB_ARRAY_TYPE__##_search( \
    struct __NB_ARRAY_TYPE__ * array, \
    __NB_ARRAY_BLOCK_TYPE__ * ptr_a, \
    nb_compare_fn compare_fn \
  ) { \
    return *__NB_ARRAY_TYPE__##_search_ptr(&array->buffer, (void *) ptr_a, compare_fn); \
  } \
  \
  __NB_ARRAY_BLOCK_TYPE__ * __NB_ARRAY_TYPE__##_search_ptr( \
    struct __NB_ARRAY_TYPE__ * array, \
    __NB_ARRAY_BLOCK_TYPE__ * ptr_a, \
    nb_compare_fn compare_fn \
  ) { \
    return (__NB_ARRAY_BLOCK_TYPE__ *) nb_search(&array->buffer, (void *) ptr_a, compare_fn); \
  } \
  \
